# Agent-01 - nmap_service

## Scope
- Tool ID: `nmap_service`
- Tool Name: `NMAP SERVICE SCAN`
- Base Command: `nmap`
- Parser Hint in tools.json: `nmap`
- Input Mode: `domain`
- Types: domain, ipv4, cidr

## Command Template
`nmap -sV -sC -T4 {target} -oN {log_file}`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `nmap_service.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/nmap_service.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/nmap_service/`
