...
## Command Template
`echo {target} | waybackurls | tee {log_file}`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `waybackurls.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/waybackurls.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/waybackurls/`
