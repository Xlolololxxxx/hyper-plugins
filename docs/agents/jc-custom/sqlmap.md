...
- Tool Name: `SQLMAP`
- Base Command: `sqlmap`
- Parser Hint in tools.json: `sqlmap`
- Input Mode: `url`
- Types: url, domain, domain_port, ip_port, ipv4

## Command Template
`sqlmap -u "{target}" --batch --random-agent --level=2 --risk=2 | tee {log_file}`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `sqlmap.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/sqlmap.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/sqlmap/`
