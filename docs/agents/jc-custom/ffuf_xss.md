...
## Command Template
`ffuf -u {target}/?q=FUZZ -w {wordlist_file} -o {log_file} -of json`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `ffuf_xss.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/ffuf_xss.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/ffuf_xss/`
