# Agent-31 - hydra_http_get_combo

## Scope
- Tool ID: `hydra_http_get_combo`
- Tool Name: `HYDRA HTTP GET BASIC`
- Base Command: `hydra`
...
## Command Template
`hydra -C {wordlist_file} -s 80 {target} http-get / -I -f -o {log_file}`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `hydra_http_get_combo.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/hydra_http_get_combo.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/hydra_http_get_combo/`
