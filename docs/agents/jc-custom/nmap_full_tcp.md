# Agent-02 - nmap_full_tcp

## Scope
- Tool ID: `nmap_full_tcp`
- Tool Name: `NMAP FULL TCP`
- Base Command: `nmap`
...
## Command Template
`nmap -p- -T4 --min-rate 1000 {target} -oN {log_file}`
...
## Deliverables
- New parser module in `local/hyper-target-panel/lib/jc-adapters/` named `nmap_full_tcp.js`
- Parser unit tests in `local/hyper-target-panel/__tests__/jc-adapters/nmap_full_tcp.test.js`
- 5 baseline fixtures from real tool output under `local/hyper-target-panel/data/jc-baseline/nmap_full_tcp/`
